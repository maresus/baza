...
from app.services.reservation_service import ReservationService, DINING_ROOMS
...
# --- Zdravstveni center overrides (ne uporabljaj kovačnik vsebin) ---
INFO_RESPONSES = {
    "pozdrav": """Pozdravljeni pri zdravstvenem centru! 😊

Lahko pomagam z informacijami o pregledih, naročanju, ceniku ali kontaktih.""",
    "kdo_si": """Sem digitalni pomočnik zdravstvenega centra.

Pomagam z informacijami o pregledih, naročanju in storitvah.""",
    "odpiralni_cas": "Delovni čas je naveden v informacijah zdravstvenega centra. Želite, da poiščem točen termin?",
    "kontakt": "Kontakti (telefon/email) so v informacijah zdravstvenega centra. Želite, da jih navedem?",
    "lokacija": "Lokacija je navedena v informacijah zdravstvenega centra. Želite, da jo poiščem?",
    "narocanje": "Za naročanje mi napišite vrsto pregleda/posega, želeni termin in kontakt.",
    "cenik": "Cenik je objavljen med informacijami zdravstvenega centra. Želite, da poiščem točno storitev?",
    "default": "Kako vam lahko pomagam? Lahko vprašate o pregledih, naročanju, ceniku ali kontaktih.",
}
INFO_RESPONSES_VARIANTS = {key: [value] for key, value in INFO_RESPONSES.items()}
BOOKING_RELEVANT_KEYS = {"narocanje"}
CRITICAL_INFO_KEYS = {
    "odpiralni_cas",
    "kontakt",
    "lokacija",
...
def _llm_system_prompt_full_kb(language: str = "si") -> str:
    common = (
        "Ti si asistent zdravstvenega centra. Odgovarjaj na podlagi preverjenih informacij.\n\n"
        "Tukaj so VSE informacije o zdravstvenem centru:\n"
        f"{FULL_KB_TEXT}\n\n"
        "Ne izmišljuj si podatkov.\n"
        "Ne podajaj diagnoz ali medicinskih nasvetov; podajaj le informativne odgovore in usmeritve.\n"
        ...
        "Če se podatki v virih razlikujejo, uporabi informacije iz baze.\n"
        "Če uporabnik želi naročiti termin, OBVEZNO pokliči funkcijo "
        "`reservation_intent` in nastavi ustrezen action.\n"
    )
    if language == "en":
        return (
            "You are the assistant for a health center. Respond in English.\n"
            + common
        )
    if language == "de":
        return (
            "Du bist der Assistent eines Gesundheitszentrums. Antworte auf Deutsch.\n"
            + common
        )
    return (
        common
    ...
    tools = [
        {
            ...
            "description": "Ugotovi ali uporabnik želi naročiti termin/pregled (rezervacija). Vrni action.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {
...
def detect_intent(message: str, state: dict[str, Optional[str | int]]) -> str:
    ...
    # zdravstveni center: posebnega "room info" ni
    ...
    if has_rezerv and (has_soba or has_miza):
        return "reservation"
    ...
    # zdravstveni center: brez sobe info
...
def detect_info_intent(message: str) -> Optional[str]:
    ...
    if any(w in text for w in ["kontakt", "telefon", "email", "e-mail", "epošt", "e-pošt", "številk", "stevilk"]):
        return "kontakt"
    if any(w in text for w in ["kje ste", "naslov", "lokacij", "google maps", "koordinate", "gps"]):
        return "lokacija"
    if any(w in text for w in ["naročanje", "narocanje", "kako do termina", "termin", "pregled", "poseg"]):
        return "narocanje"
    if "cenik" in text or "cena" in text:
        return "cenik"
...
def get_booking_continuation(step: str, state: dict) -> str:
    ...
    continuations = {
        ...
        "awaiting_table_location": "Katero **vrsto pregleda/posega** želite?",
...
def detect_router_intent(message: str, state: dict[str, Optional[str | int]]) -> str:
    ...
    if has_table and any(tok in lower for tok in ["oseb", "ob ", ":00", "termin"]):
        return "booking_table"
...
def last_bot_mentions_reservation(last_bot: str) -> bool:
    text = last_bot.lower()
    return any(token in text for token in ["rezerv", "reserve", "booking", "pregled", "poseg", "termin", "appointment"])
...
def reservation_prompt_for_state(state: dict[str, Optional[str | int]]) -> str:
    ...
    if res_type == "table":
        if step == "awaiting_table_date":
            return "Prosim za datum v obliki DD.MM.YYYY."
        if step == "awaiting_table_time":
            return "Ob kateri uri želite termin? (npr. 09:30)"
        if step == "awaiting_table_people":
            return "Za koliko oseb (pacientov) naj rezerviram termin?"
        if step == "awaiting_table_location":
            return "Izberi vrsto pregleda/posega (npr. Ortopedski pregled, Dermatološki pregled)."
    else:
...
def handle_inquiry_flow(message: str, state: dict[str, Optional[str]], session_id: str) -> Optional[str]:
    ...
    if step == "awaiting_contact":
        ...
        send_custom_message(
            INQUIRY_RECIPIENT,
            "Novo povpraševanje – zdravstveni center",
            summary,
        )
        reset_inquiry_state(state)
...
def generate_confirmation_email(state: dict[str, Optional[str | int]]) -> str:
    subject = "Zadeva: Rezervacija – zdravstveni center"
    ...
    if state.get("type") == "room":
        ...
        lines.append(
            f"Prejeli smo rezervacijo termina za {state.get('people')} oseb na datum {state.get('date')} ob {state.get('time')}."
        )
        if state.get("location"):
            lines.append(f"Vrsta pregleda/posega: {state.get('location')}.")
...
def table_intro_text() -> str:
    categories = ", ".join([r["name"] for r in DINING_ROOMS])
    return f"Na voljo so naslednje vrste pregledov/posegov: {categories}."
...
def parse_reservation_type(message: str) -> Optional[str]:
    ...
    # pregled / poseg - slovensko, angleško, nemško
    table_keywords = [
        "pregled",
        "poseg",
        "termin",
...
def _handle_table_reservation_impl(message: str, state: dict[str, Optional[str | int]]) -> str:
    ...
    def proceed_after_table_people() -> str:
        appointment_types = [r["name"] for r in DINING_ROOMS]
        reservation_state["available_locations"] = appointment_types
        reservation_state["step"] = "awaiting_table_location"
        return "Katero vrsto pregleda/posega želite? (" + ", ".join(appointment_types) + ")"

    if step == "awaiting_table_date":
        proposed = extract_date(message) or ""
        if not proposed:
            return "Za kateri datum želite termin? (DD.MM.YYYY)"
        ok, error_message = reservation_service.validate_table_rules(proposed, "09:00")
        if not ok:
            reservation_state["date"] = None
            return error_message + " Prosim pošljite datum v obliki DD.MM.YYYY."
    ...
    if step == "awaiting_table_time":
        ...
        if not ok:
            ...
            return error_message + " Poskusiva z novim datumom (DD.MM.YYYY)."
    ...
    if step == "awaiting_note":
        ...
        lines = [
            ...
            f"👥 Osebe: {reservation_state.get('people')}",
            f"🩺 Vrsta pregleda/posega: {reservation_state.get('location')}",
            f"👤 Ime: {reservation_state.get('name')}",
            f"📞 Telefon: {reservation_state.get('phone')}",
            f"📧 Email: {reservation_state.get('email')}",
        ]
    ...
    if step == "awaiting_confirmation":
        ...
        if is_affirmative(message):
            ...
            reset_reservation_state(state)
            final_response = (
                "Super! 😊 Vaša rezervacija termina je zabeležena:\n"
                f"📅 Datum: {summary_state.get('date')} ob {summary_state.get('time')}\n"
                f"👥 Osebe: {summary_state.get('people')}\n"
                f"🩺 Vrsta pregleda/posega: {summary_state.get('location')}\n"
    ...
    if step == "awaiting_table_people":
        ...
        if people is None or people <= 0:
            return "Prosim sporočite število oseb (npr. '1 oseba')."
    ...
    if step == "awaiting_table_location":
        ...
        reservation_state["step"] = "awaiting_name"
        return f"Zabeleženo: {selected}. Prosim še ime in priimek pacienta."
    ...
    if step == "awaiting_email":
        ...
        reservation_state["email"] = email
        reservation_state["step"] = "awaiting_note"
        return "Želite še kaj sporočiti? (npr. posebne želje ali težave)"

    return "Nadaljujmo z rezervacijo termina. Kateri datum vas zanima?"
...
def handle_reservation_flow(message: str, state: dict[str, Optional[str | int]]) -> str:
    ...
    if detect_reset_request(message):
        reset_reservation_state(state)
        return _tr("Ni problema, začniva znova. Želite rezervirati termin za pregled ali poseg?")

    # če smo v enem toku, pa uporabnik omeni drug tip, preklopimo
    lowered = message.lower()
    if reservation_state["step"] and reservation_state.get("type") == "table" and any(
        token in lowered for token in ["pregled", "poseg", "termin"]
    ):
        reservation_state["step"] = "awaiting_table_date"
        return _tr(
            f"Preklopim na rezervacijo termina. Za kateri datum? (DD.MM.YYYY)\n{table_intro_text()}"
        )
    ...
    if reservation_state["step"] is None:
        ...
        if detected == "table":
            ...
            reservation_state["step"] = "awaiting_table_date"
            return _tr(
                f"Odlično, rezervirajmo termin. Za kateri datum? (DD.MM.YYYY)\n{table_intro_text()}"
            )
        reservation_state["step"] = "awaiting_type"
        return _tr("Kako vam lahko pomagam – rezervacija termina za pregled ali poseg?")

    if reservation_state["step"] == "awaiting_type":
        choice = parse_reservation_type(message)
        if not choice:
            return _tr(
                "Mi zaupate, za kateri pregled ali poseg želite termin? "
                f"{table_intro_text()}"
            )
        reservation_state["type"] = choice
        reservation_state["step"] = "awaiting_table_date"
        return _tr(
            f"Super, uredim termin. Za kateri datum? (DD.MM.YYYY)\n{table_intro_text()}"
        )
...
def chat_endpoint(payload: ChatRequestWithSession) -> ChatResponse:
    ...
    if state.get("step") is None and is_affirmative(payload.message):
        ...
        if last_bot_mentions_reservation(last_bot):
            if any(token in last_bot for token in ["pregled", "poseg", "termin", "appointment"]):
                state["type"] = "table"
            else:
                state["type"] = None
    ...
    if state.get("step") is None:
        ...
        has_table_context = any(token in last_bot for token in ["pregled", "poseg", "termin", "appointment"])
    ...
    if USE_FULL_KB_LLM:
        ...
        if is_ambiguous_reservation_request(payload.message):
            reply = "Želite naročiti termin za **pregled/poseg**?"
            reply = maybe_translate(reply, detected_lang)
        ...
        if any(token in payload.message.lower() for token in fallback_tokens) or is_reservation_typo(payload.message):
            reset_reservation_state(state)
            state["type"] = "table"
            reply = handle_reservation_flow(payload.message, state)
            return finalize(reply, "booking_appointment_fallback", followup_flag=False)
        llm_reply = _llm_answer_full_kb(payload.message, detected_lang)
